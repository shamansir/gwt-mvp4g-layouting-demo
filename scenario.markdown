...
### 4a. Making a complete page without state support
 ...
 1. Register a navigation event for your page in the event bus of the appropriate module. See methods `news()`, `edit()` и `show()` in [page/news/NewsEventBus.java][NewsEventBus] interface.
 1. Add a call of this event to the `convertFromUrl()` method of your module's `HistoryConverter`, in case if this portal was received. Create an `on...` handler method in converter for your event and return the converted parameters, if there any exist — you can use `url` property of converter for your convenience to construct/parse parameters. See example [page/news/history/NewsHistoryConverter][NewsHistoryConverter].
 ...
 1. Create a `View` for your page, inherit from [Portal][] class. For every part of a page that planned to be inserted into the separate placeholder (a widget) you need to use a wrapping [Plug][] widget for each of those, regardless of whether you use `ui.xml` or not. The root of such `view` must be a [Plugs][] widget that allows to enumerate any number of `Plug` widgets inside it. See example [page/news/view/NewsEditView.java][NewsEditView] and [page/news/view/NewsEditView.ui.xml][NewsEditView.ux], in case of `NewsEditView` the `infoPlug` (information block) и `savePlug` ("Save" button) widgets will be placed in different placeholders.
