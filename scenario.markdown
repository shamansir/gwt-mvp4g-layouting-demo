...
In fact, in [`name.shamansir.mvplayout.lib`](https://github.com/shamansir/gwt-mvp4g-layouting-demo/tree/master/src/name/shamansir/mvplayout/lib) package there is a code you may use as a library with ease. I'm not wrapping it with `jar` package, but potentially it is // for usage — a complex example works stable, and the core idea (not actual one, however) works in
